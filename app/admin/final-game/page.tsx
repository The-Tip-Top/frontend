...
const FinalGame = () => {
  ...
  useEffect(() => {
    startTransition(() => {
      getAllTicketsWithParticipations().then((res) => {
        setData(res ?? [])
      }).catch((err) => {
        console.log(err)
      })
      ...
      <div className="home-content">
        <header className="home-header relative">
          <HeaderContainer
            title="Bienvenue"
            user={''}
            subtext="Bienvenue dans l'éspace grand jeux concour!"
          />
        ...
        <div className="recent-gifts mb-12 relative h-[calc(100vh-200px)] w-full">
          <div className="flex h-full flex-col gap-y-4 justify-center items-center">
            {
              isPending ? <ClipLoader className="w-30 h-30" /> :
            <Card className="mt-6 text-xl font-bol w-[50%] p-3" hidden={done} >
              ...
              <CardFooter className="mt-3">
                <Button variant={"default"} size={"sm"}
                  ...
                  className=" text-white w-full bg-[#18181B]">Tirage</Button>
              </CardFooter>
            </Card>
            }
            {done && currentNumber && data !== null && (
              <CardItem userDetails={data[currentNumber]} />
            )}
          </div>
        </div>
      </div>
...
const CardItem = ({ userDetails }: CardItemProps) => {
  return (
    ...
    >
      <Image src={'/cadeau1.webp'} width={130} height={230} alt="cadeau1" className="rounded-t-md w-full h-[230px] object-cover" />
      <div className="p-2">
        ...
        {userDetails.gifts?.map((g) => (
          <>
            ...
            <div className="flex justify-between">
              ...
              <p className="text-sm font-semibold text-gray-600 mt-4 mb-2">{formatDateTime(new Date(userDetails?.participations?.find((p) => p.participationGiftId === g.giftId)?.createdAt ?? "")).dateTime}</p>
            </div>
          </>
        ))}
